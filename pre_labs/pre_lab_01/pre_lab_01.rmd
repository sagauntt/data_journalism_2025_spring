...
**Answer** 79366 observations of 9 variables
...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer** the "id" column has a data type of <chr> while the "seats" column has a data type of <dbl>
...
**Answer** The first line tells R to pull from the umd_courses data, the second line groups all observations from the same term together, then the third line counts how many total classes are in each term. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** The result is now organized by the amount of classes from most to least, rather than by term. It changed the result by excluding some rows that were previously included when grouped by term, and including some new rows once grouped by class count. 
...
**Answer**The data is now grouped by both term and the class department. The result now includes each department and the amount of classes offered in them each term. The code changed by grouping by department in addition to term, and adding the line to arrange by term and department. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** The code  now includes the total number of seats offered per term, and does not include the class department. The results are organized by the amount of total seats offered each term from most to least. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer** The code changed by adding the lines to include the mean and median number of seats in the results. Because the code is still sorting by total number of seats, the top terms have not changed order but they do now include the mean and median number of seats per class in the results. 
...
**Answer** The code changed by grouping by department rather than term, and including lines to determine the minimum and maximum seat numbers by department. The data is sorted by the departments with most seats to least seats. 
...
1.  Over previous semesters, which department has lost the most numbers of seats? 

2.  Which class has the most number of seats? 

3.  How has the median seat number across all university departments changed over time? 


**finished my hw, see you in class!**
