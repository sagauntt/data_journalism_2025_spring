...
**Answer**The first line of the error message is "One or more parsing issues, call `problems()` on your data frame for details, e.g.:". When running problems(payments), the result is a table with all the parsing problems in the data set. There are 369 rows of issues. 
...
**Answer** No, there are 0 parsing problems when loading the data. 
...
**Answer**The "Vendor Name" field is <chr> data type, the "Amount" field is a <dbl> data type and the "Date" column is a <chr> data type. 
...
**Answer**There are 553 listed Zip codes with less than 5 characters long. Some also include letters. I would imagine this is mostly due to input error from missing a number or accidentally filling the wrong field, but as many of entries below are 4 characters long perhaps some came from people trying to input the +4 extension to a Zip code.
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **Answer** The codeblock is taking the md_grants_loans data and grouping it by the "Fiscal Year" and Category values, adding up each value of the Category column by fiscal year, and then arranging the data by fiscal year. For most fiscal years, the value of the category column is either "Grant" or "Loan," but there are some entries that have no category listed and are just NA. Additionally, there is one entry where the category is listed as a contract in fiscal 2017, and in 2013 some loans are listed as just "L". 
...
**Answer** The two smallest listed loans are for 60 and 95.98 dollars, respectively. But after that, the loans quickly jump to several hundred dollars where the increases in grant amount then become a bit more gradual. While the loans are extremely small, they don't strike me as questionable or outliers.  
...
**Answer**It changes the headers so they are all lowercase and replaces spaces with _
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **Answer** clean_names() is fixing the capitalization and space issues in the column names, rename(source = grantor) is renaming the "grantor" column to "source," the mutate(source = str_to_upper(source) line is changing all text in the source column to be all uppercase, the grantee = str_to_upper(grantee) line is changing text in the grantee column to be all uppercase, and finally the description = str_to_upper(description)) line is changing all text in the description column to be all uppercase. 
...
**Answer** There are 58 duplicates possible. 
...
**Answer** The new dataframe has 19,453 rows. 
...
**Answer** The original zip codes weren't deleted, but there is now a new zip5 column which has the converted 5-digit zip code. 
...
**Answer** The 21202 Zip code received the most total funds from the State Tourism Board, at 648069 dollars. This makes sense, as the Zip code encompasses Downtown Baltimore and the Inner Harbor. 

```{r}
cleaned_md_grants_loans |>
  filter(source=="COMMERCE/MARYLAND TOURISM BOARD") |>
  group_by(zip5) |>
  summarise(total_funds=sum(amount)) |>
