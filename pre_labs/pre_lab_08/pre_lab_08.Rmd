...
**Answer** The code is creating a new column with calls by month. The label=TRUE is labeling each month as their name (dec., jan., etc.) rather than just their corresponding number. It is then grouping the data by the month, and summarizing it to include the total number of calls per month and sorting it by the months with the most calls first. 
...
**Answer**After running the code, I just see a blank canvas 
...
**Answer**After running the code, it now shows a bar graph with the y axis being total calls and each bar being a month in the year. 
...
**Answer** It now shows a graph with the same information, but the bars are sorted by least to most calls rather than chronologically. 
...
**Answer**Now, the graph's orientation has shifted. The X-axis is measuring total calls now and the bars are stacked vertically. 
...
**Answer**The theme is now different, with a slightly less noticeable background and more clearly-labeled values. 
...
**Answer**The theme has shfited again. It now has a light blue background, the spacing is different and the fonts are a little larger. 
...
**Answer**The graph now has the title "More 911 Overdose Calls in Warmer Months," a source tag, and the y and x axis are labeled more specifically. 
...
**Answer**The code shows total calls in Baltimore County on each date of the year. 
...
**Answer**It shows a ling chart with each point connected to the next value. 
...
**Answer**The graph now lists each week of data, but there are too many weeks on the graph so they all overlap and are unreadable. 
...
**Answer**The angle of the weeks is adjusted so they have an upward slant and each can fit on the graph without overlapping. They're still not too pretty, though. 
