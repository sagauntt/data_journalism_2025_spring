...
**Task** Run the following code and describe the dataframe it outputs. **Answer** The result is a dataframe where each row is a disciplinary alert against an individual practitioner and each column is a different piece of information, such as the case id, url, first name, last name, type of discipline, year and the actual text of the alert. 
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer**The function lists each occurrence of a unique entry. It combines the double entries of "dog" and "cat." so it only shows "Dog," "dog," "cat," and "," 
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer**The function creates a list of each unique word/entry in the alerts dataframe in a single column. 
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** The function creates a bar plot of each unique word in order of which was most used. Basic words consist of most of the list. The graph shows that "the" was by far the most commonly-used word, at almost 100,000 instances. 
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** The result no longer shows the stop words. It is definitely more interesting than before, as it includes words like "discipline," "patient," "consent," etc. 
...
**Task** Run the following code and describe the results. **Answer**The two lists are mostly similar, but they have some differences and have a different order. For example, only the 2023 results contains "hearing" and only the 2022 results show "care"
...
**Task** Run the following code and describe the results. **Answer**The list now shows commonly-used two-word phrases, like "summary suspension" and "unprofessional conduct." The results are starting to get more specific, and with that, they're getting more interesting. 
...
**Task** Run the following code and describe the results. **Answer**The results of common two-word phrases from 2023 are the same as those from 2021. 
...
**Task** Run the following code and describe the results. **Answer**The results are now slightly different, and feature new phrases like "prehearing conference" or "respondent's license." Percentages for each result are quite small, with the highest being around 1 percent. 
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer**The results mostly make sense, but certain words caught my eye. For example, "appeal" doesn't necessarily seem like it would have a positive connotation to me. Additionally, some of these words on their own could have different connotations from the context in which they were used. "Substantive" on its own may be positive, but a phrase like "the allegations were substantive" would likely have a negative connotation. So judging each word on its own could potentially miss the larger picture. 
