...
```{r}
voters_by_county <- read.csv("tabula-Eligible Active Voters by County - GG22.csv")
...
**Answer** There are still lines of dots on the area entries, and the columns dont have headers they are just the first row of data for Maryland as a whole.
...
**Answer**Maryland is now included in the row of data and the columns have headers, but they aren't descriptive and don't tell readers what the chart is measuring. There are also still dots in the entries in the first column. 
...
**Answer** The columns are now properly named, but the dots in the first column remain. We need to remove those. 
