...
**Answer** It's important to create a new variable to hold these results because we may wish to do further analysis with more information than just from those two columns, and we want to be able to pull from the journalism courses information again without overriding our earlier results. 
...
**Answer** The OR operator gives 54000 results. It is a list of all classes in the Theatre department and all classes across the entire university with 15 or more seats. 
...
**Answer:** The dates are now in the y/m/d format instead of m/d/y, and it is also separated by dashes instead of slashes. 
...
**Answer** The month column now has grouped all individial rows of data from that month into the single variable. So the row "2022-06-01" contains all data for the month of June, 2022. 
...
**Answer** The code mutates the general_22 data to add a new column which shows the percent of the vote that went for Moore in each county. 
...
**Answer** The data is now listed by the counties that had the highest percentage of the vote going toward Moore rather than alphabetically. It is changed because we used the arrange function to organize it by the pct_moore column. 
...
**Answer** While I don't think its necessarily a more useful answer, I do think it is interesting. Since Maryland is such a strong Democratic state, Moore's victory was all but assured. Prince George's, Baltimore, Howard and Montgomery are all known and reliable strongholds. So it makes sense to want to see where he performed worst. 
...
**Answer** str_to_lower shifts all text to lowercase, while str_to_title shifts to title case, where the first letter of each word is capitalized. 
...
**Answer**The Mutate function is creating a new column "state" by using the str_detect function to find when the listed address includes a state such as Maryland "MD" or Florida "FL". It is also setting the default to "NA" for entries that don't have a state listed in the address. 
...
**Answer**It does, it is listing the total amount spent by the what state was listed for each payee. 
...
1.  How many classes in 2024 has the term "DEI" in its course description? 

2.  How many classes in 2025 has the term "DEI" in its course description?

3.  How many classes are offered with fewer than 15 seats? 
