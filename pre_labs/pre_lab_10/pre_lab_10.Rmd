...
**Answer** The data includes zip codes listed by an object ID, traditional five-number zip, and the zip's primary community. It also includes each area's length, overall area and geometry. 
...
**Answer** The output is a list of Maryland zip codes. It includes the traditional zip code, a longer version listed as "zipcode2," and an object ID. It includes which community the zip code contains, its length and area, as well as its "geometry." The geometry column lists shapes at multipolygons and contains a list of values separated by commas. 
...
**Answer** The code creates a map of Maryland zip codes on a white background with degrees of latitude and longitude. 
...
**Answer** The code loads tigris and also shows this tip: To enable caching of data, set `options(tigris_use_cache = TRUE)`
...
**Answer**The code shows values for all U.S. Counties, 
...
**Answer**The dataset is now filtered to include only Maryland's 24 counties. 
...
**Answer**The map now includes only the county boundaries. It does now show zip codes. The map also doesn't differentiate between land and water. 
...
**Answer**The map now shows both county and zip code boundaries. Because the zip codes don't extend over water, it is now slighlty clearer which areas are rivers/the bay. 
...
**Answer**The code lists the total amount of foreclosure notices in each zip code during March 2023. 
...
**Answer**The code produces the original data of Maryland zip codes but now includes the column for the total amount of foreclosure notices in March 2023 for each zip. 
...
**Answer** The code produces a map of Maryland with each zip color coded by how many foreclosures notices they had in March 2023. The lighter the color, the more notices the zip had. 
...
**Answer**The code loads the libraries we need and sets the census API key. 
...
**Answer**The code creates two separate lists which contain the  amount of foreclosure notices during March 2023 and the population for each Maryland county. 
```{r}
...
**Answer**The code creates the same list of each Maryland county by population from census numbers, but it now also includes the county's geometry. 
...
**Answer**The code shows the names of each column and values for md_county_population and md_county_notices. 
...
**Answer**The code mutates each dataset so their column names match and can be joined. 
...
**Answer**The code joins the population and foreclosure data, creating a new singular dataset called md_pop_with_foreclosures
...
**Answer**The code creates a new column which contains the rate of foreclosure per 1000 residents in the county. 
...
**Answer**The code creates a choropleth map of Maryland counties which is color coded by the rate of foreclosure per 1000 residents. It also shows the name of each county. 
...
**Answer**The code removes the county names from the map and changes its color scheme. 
...
**Answer**The code further condenses the color values to better highlight which counties have higher foreclosure rates. 
