...
PG_911_calls <- read_csv("~/Documents/GitHub/data_journalism_2025_spring/labs/lab_08/data/prince_georges_2022_overdoses.csv") |> clean_names() 

PG_911_calls <- PG_911_calls |> mutate(month = month(datetime, label=TRUE), week = week(datetime), date=date(datetime)) 
...
PG_911_calls |> group_by(month) |>
  summarise(total_calls=n()) |>
  arrange(desc(total_calls)) |>
  ggplot() + geom_bar(aes(x=month, weight=total_calls)) + labs(
...
PG_911_calls |> group_by(week) |>
  summarise(total_calls=n()) |>
  arrange(desc(total_calls)) |>
  ggplot() + geom_line(aes(x=week, y=total_calls)) + labs(
    title="Calls by Week",
    x="Week",
    y="Number of Calls",
    caption="Source: Prince George's County EMS") 
