...
**A1**The Computer, Math, and Natural Sciences has the highest average number of seats for its classes. I found it surprising that the air science department has a mean of 120 seats. 
...
**A2**The greatest number of times a class is offered is for AASP100.
...
**A3** Geographical Sciences Professor Keith Yearwood has taught the most classes, at 79. 
...
**A4** The results include answers on the class instructor that aren't actually instructors, such as "NA" or "Instructor: TBA". Being able to eliminate specific responses from that column and sort so it is only actual names would have made finding the result easier. I am fairly confident that answer is correct, as I sorted the information to include both the class instructor and department, and sorted by amount of classes from most to least, using the count_id variable I created in the previous question. 
...
**A5** My question was "Which class has the most number of seats?" According to my analysis below, ENES140 has the most seats of any other class, with a total of 31,451 seats offered over all time.
