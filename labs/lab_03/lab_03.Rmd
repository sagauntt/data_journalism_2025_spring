...
```{r}
pg_schools <- cleaned_payments <- payments |> clean_names() |> filter(vendor_name == "PRINCE GEORGES COUNTY PUBLIC SCHOOLS" | vendor_name == "PRINCE GEORGE'S COUNTY PUBLIC SCHOOLS" | vendor_name == "PRINCE GEORGES COUNTY PUBLIC SCHOOL" |vendor_name == "PG COUNTY PUBLIC SCHOOL" | vendor_name == "PRINCE GEORGES PUBLIC SCHOOLS" | vendor_name == "PRINCE GEORGES COUNTY SCHOOL")
```
...
```{r}
UMD_capital_projects <- cleaned_payments |> filter(agency_name =="CAPITAL PROJECTS- UNIVERSITY OF MARYLAND") |>
...
```{r}
stem_cell_grants <- md_grants_loans |> filter(str_detect(Description, "STEM CELL") | str_detect(Description, "Stem Cell")) |>
...
```{r}
UMCP_zip <- cleaned_md_grants_loans <- md_grants_loans |> clean_names() |> filter(zip_code == "20740" | zip_code == "20742")
```
